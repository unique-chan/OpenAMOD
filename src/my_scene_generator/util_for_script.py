...
# Depricated after increased size of Screenshot folder
# def insert_start_flag_sign_waiting_code(scr, flag_name, sleep=1):
#     scr.add(f'sleep {sleep}')
#     scr.add(f'''KK_fnc_fileExists = {{ 
...
#     my_flag = true;
#     while {{my_flag}} do {{
#         if ("start{flag_name}.dummy" call KK_fnc_fileExists) then {{
#             my_flag = false;
#         }};
#     }};
#     sleep ''' + f'{sleep};')

# Lefted because we may use getAllEnvSoulndControlers
# maybe we can use surfaceType()
# def insert_each_position_land_sea_check_code(scr, cx, cy, ixiys):
...
#         }} else {{
#             is_sea_{i} = false;
#             my_var_{i} = "CBA_I_InvisibleTargetVehicle" createVehicle([{cx + ix}, {cy + iy}]);
#             is_land_{i} = false; 
#             tmp = str(getAllEnvSoundControllers position my_var_{i});
#             tmp = [tmp, "],"] call BIS_fnc_splitString;
#             if ( (parseNumber(tmp select 11) > 0) 
#                             and (parseNumber(tmp select 13) == 0)
#                             and (parseNumber(tmp select 15) == 0) 
#                             and (parseNumber(tmp select 17) == 0) ) then {{
#                                 is_land_{i} = true;
#                             }};
#             deleteVehicle my_var_{i};
#         }}''')
...
def insert_item_placement_code(self, scr, i, ix, iy, rot_degree, land_item, sea_item, item_radius, i_time, z_atl=2):
    ...
    scr.add(f'nos = nearestObjects [[{ix}, {iy}, 0], [], {item_radius * 0.8}, true]') # change 2.0 to 0.8
    scr.add_no_col_end(f'''if (count(nos) == 0) then {{ if (is_land_{i}) then {{         
        my_var_{i} = createVehicle ["{land_item}",[{ix}, {iy}, {z_atl}],[],0,"NONE"];
    ...
    if sea_item is not None:
        scr.add(f'''else {{
            if (is_sea_{i}) then {{
                my_var_{i} = createVehicle ["{sea_item}",[{ix}, {iy}, {z_atl}],[],0,"NONE"];
                my_var_{i} setCombatBehaviour "CARELESS";
                my_var_{i} setVehicleTIPars [{engine_tmp}, {tire_tmp}, {firearm_tmp}];
                my_var_{i} addEventHandler ["hit", {{
...
def insert_random_items_placement_code(self, scr, land_or_sea, sampled_item_classes, cx, cy, ixiys, i_time):
    land_sea_item_pairs = []
    ...
    for cnt, item in enumerate(sampled_item_classes):
        ...
        insert_item_placement_code(self, scr, cnt, cx + ix, cy + iy, rot_degree,
...
def check_item_collision(mask, ix, iy, radius):
    try:
        region = mask[iy - radius:iy + radius, ix - radius:ix + radius]
    except:
        return True
    region_one_hot = where(region == 1)
    if len(region_one_hot[0]) or len(region_one_hot[1]):
        return True
    mask[iy - radius:iy + radius, ix - radius:ix + radius] = 1
    return False
...
def insert_camera_creation_code(scr, cam_point, fov=0.75):
    ...
    scr.add(f'cam = "camera" camCreate [{cam_point[0]}, {cam_point[1]}, {cam_point[2]}]')
...
def insert_camera_tilting_code(scr, target_point, sleep=0.8):
    scr.add(f'cam camSetTarget [{target_point[0]}, {target_point[1]}, {target_point[2]}]')
